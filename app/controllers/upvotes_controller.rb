class UpvotesController < ApplicationController
  ...
  def create
    ...
    if params[:story_id] && !params[:sentence_id] #checking if its a story upvote or sentence upvote
      if Upvote.all.select {|vote| vote.user_id == current_user.id && vote.upvotable_id == params[:story_id].to_i }.empty? #making sure user hasnt already voted
      ...
      if Upvote.all.select {|vote| vote.user_id == current_user.id && vote.upvotable_id == params[:sentence_id].to_i }.empty?
